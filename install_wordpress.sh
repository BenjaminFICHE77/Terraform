...
#change root password to db_host
#  mysql -p$temppassword --connect-expired-password  -e "SET PASSWORD FOR root@localhost = PASSWORD('$db_host');FLUSH PRIVILEGES;" 
# mysql -p'$db_host'  -e "DELETE FROM mysql.user WHERE User='';"
# mysql -p'$db_host' -e "DELETE FROM mysql.user WHERE User='root' AND Host NOT IN ('localhost', '127.0.0.1', '::1');"
# Create database user and grant privileges
# mysql -u root -p"$db_host" -e "GRANT ALL PRIVILEGES ON *.* TO '$db_username'@'localhost' IDENTIFIED BY '$db_user_password';FLUSH PRIVILEGES;"
# Create database
# mysql -u $db_username -p"$db_user_password" -e "CREATE DATABASE $db_host;"
